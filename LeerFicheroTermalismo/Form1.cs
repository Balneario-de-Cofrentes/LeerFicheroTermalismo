    ...
    public partial class Form1 : Form
    {
        ...
        public void LeerDatosExcel(string fichero)
        {
            ...
            foreach (DataRow dr in dataSet.Tables[0].Rows)
            {
                ...
                else
                {
                    ...
                    if (System.Configuration.ConfigurationSettings.AppSettings["comprueba_reservas"] == "1")
                    {
        ...
        public int ExistePrereserva(int HUE_GUID, string[] huesped, string nif_huesped, string nif_original)
        {
            ...
            if (this.checkBox3.Checked)
            {
                ...
                //sql = "SELECT edi_des_str,res_nal_bln,age_nom_str,res_DR_bon_str,DateDiff(day, res_ent_dat, res_sal_dat) as DiferenciaDias,res_spe_adu_int,[PresReservas].RES_GUID,HuespedK.HUE_GUID,PresReservaAcompanantes.HUE_GUID as acomp,hue_nif_str,res_ent_dat,PreReservaEstados.epr_des_str as Estado,prr_est_byt FROM PresReservas INNER JOIN  PreReservaEstados ON PreReservaEstados.epr_GUID=PresReservas.epr_GUID INNER JOIN HuespedK ON HuespedK.HUE_GUID = PresReservas.HUE_GUID LEFT  JOIN  PresReservaAcompanantes ON  PresReservaAcompanantes.RES_GUID = PresReservas.RES_GUID   INNER JOIN Agencias ON  Agencias.AGE_GUID = PresReservas.AGE_GUID  LEFT JOIN Edificios ON Edificios.EDI_GUID  = PresReservas.EDI_GUID  WHERE  PresReservas.AGE_GUID='" + this.agencia_balneario + "' AND HuespedK.hue_nif_str='" + Convert.ToString(nif_huesped) + "' AND (res_ent_dat between '" + Convert.ToDateTime("01/01/" + Convert.ToDateTime(VectotConvocatorias[1]).Year).ToString(formato_datetime) + "' AND   '" + Convert.ToDateTime("31/12/" + Convert.ToDateTime(VectotConvocatorias[1]).Year).ToString(formato_datetime) + "'  ) ";
            ...
            /*if (this.checkBox3.Checked)
            {
                ...
                string[] ExpedienteAno = huesped[0].Trim().Split(delimiters_re);
                this.misfuncionesBD.update("PresReservas", "res_DR_bon_str='" + ExpedienteAno[1] + "'", "RES_GUID=" + RES_GUID);
                ...
                string[] ExpedienteAno = huesped[0].Trim().Split(delimiters_re);
                this.misfuncionesBD.update("PresReservas", "res_DR_bon_str='" + ExpedienteAno[1].Trim() + "'", "RES_GUID=" + RES_GUID);
        ...
        public void InsentarAdvertencias(DateTime FechaImserso, DateTime FechaAci, string Tabla, string IdHuespedAci, string NifSolictenteImnserso, string NifHuespedAci, string IdRererva, string Expediente, string estado, string numeroadultos, string plazassolicitadas, string nif_acomp_Aci, string nif_acomp_imserso, string tipoturno, string diasestancia, string expedienteACi, string noalojados, string AgenciaBalneario, string res_bon_str)
        {
            string mesfichero = Convert.ToString(FechaImserso.Month);
            string anyofichero = Convert.ToString(FechaImserso.Year);
            string no_alojados_advertencias = "";
            ...
            if (!File.Exists(curFile))
            {
                ...
                xlWorkSheet.Cells[1, 14] = "DIAS_ESTANCIA";
                xlWorkSheet.Cells[1, 15] = "TURNO_IMSERSO";
                xlWorkSheet.Cells[1, 16] = "TIPO_ADVERTENCIA";
                xlWorkSheet.Cells[1, 17] = "OBSERVACION";
            ...
            if (noalojados == "True")
            {
                ...
                no_alojados_advertencias = " de tipo de NO ALOJADO ";

            }


            //MessageBox.Show("Imserso--> " + NifSolictenteImnserso.Trim() + "ACi--> " + NifHuespedAci.Trim());
            //Advertencias que exiete una reserva con misma fecha o distinta
            if (formatearnif(NifSolictenteImnserso.Trim()) != formatearnif(NifHuespedAci.Trim()))
            {
                ...
                if (expedienteACi == Expediente)
                {
                    ...
                    fichero_advertencias = "Expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  en estado " + estado + "  de la Agencia " + AgenciaBalneario + " con diferentes NIF en Aci y " + institucion + "  nif de solicitente " + formatearnif(NifSolictenteImnserso.Trim()) + " y nif en Aci " + formatearnif(NifHuespedAci.Trim()) + " para esta fecha " + FechaImserso.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                    ...
                    TIPO_ADVERTENCIA = "Existe el Expediente";
                    sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
            ...
            //Advertencias que exiete una reserva con diferentes peticiones de solicitudes
            if (numeroadultos == "1" && plazassolicitadas == "C")
            {
            ...
            //Advertencias que exiete una reserva con diferentes peticiones de solicitudes
            if (numeroadultos == "2" && plazassolicitadas == "A")
            {
            ...
            //Advertencias que exiete una reserva con diferentes peticiones de solicitudes
            if (numeroadultos == "2" && plazassolicitadas == "B")
            {
            ...
            //Advertencias que exiete una reserva o prereserva con diferentes acompañantes
            if (estado == "Anulada")
            {
                ...
                fichero_advertencias = "";
                fichero_advertencias = "Este expediente " + Expediente + " del solicitante " + NifSolictenteImnserso.Trim() + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado;
                ...
                TIPO_ADVERTENCIA = "Reserva Anulada";
                sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
            ...
            if (estado == "Transpaso AI")
            {

                fichero_advertencias = "Este expediente " + Expediente + " del solicitante " + NifSolictenteImnserso.Trim() + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + Environment.NewLine + Environment.NewLine;
                ojos_linea = true;
            ...
            //Advertencias de numero de dias instancia



            if (diasestancia.Trim() == "11" && tipoturno.Trim() != "B (12 DIAS)")
            {

                fichero_advertencias = "Este expediente " + Expediente + " del solicitante " + NifSolictenteImnserso.Trim() + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " con diferentes dias " + diasestancia.Trim() + " de estancia que su turno " + tipoturno.Trim() + Environment.NewLine + Environment.NewLine;
                ojos_linea = true;
                ...
                TIPO_ADVERTENCIA = "Numero de dias instancia diferentes";
                sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
            ...
            //Advertencias de numero de dias instancia
            if (diasestancia.Trim() == "9" && tipoturno.Trim() != "A (10 DIAS)")
            {
            ...
            //Advertencias de numero de dias instancia
            if (diasestancia.Trim() == "7" && tipoturno.Trim() != "A (7 DIAS)")
            {
                ...
                TIPO_ADVERTENCIA = "Numero de dias estancia diferentes";
                sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
            ...
            if (plazassolicitadas == "C")
            {
                ...
                if (formatearnif(nif_acomp_Aci.Trim()) != formatearnif(nif_acomp_imserso.Trim()))
                {
                    ...
                    fichero_advertencias = "Este expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " con diferentes NIF de Acompañante en Aci y " + institucion + " nif de Aci " + formatearnif(nif_acomp_Aci) + " y nif en " + institucion + " " + formatearnif(nif_acomp_imserso) + Environment.NewLine + Environment.NewLine;
                    ojos_linea = true;
                    ...
                    TIPO_ADVERTENCIA = "Acompañante Diferente";
                    sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
            ...
            if (FechaImserso.CompareTo(FechaAci) == 0)
            {
                ...
                fichero_advertencias_log += "Para este expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " del NIF " + NifHuespedAci + " para esta fecha " + FechaImserso.ToString("dd/MM/yyyy") + " fecha en Aci " + FechaAci.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;

                if (ojos_linea == true && Tabla == "Reserva")
                {
                    fichero_advertencias += "Para este expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " del NIF " + NifHuespedAci + " para esta fecha " + FechaImserso.ToString("dd/MM/yyyy") + " fecha en Aci " + FechaAci.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                }
            ...
            else
            {
                ...
                if (ObtenerRangoConvocatoria(FechaImserso) != ObtenerRangoConvocatoria(FechaAci))
                {

                    fichero_advertencias = "Este expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " del NIF " + NifHuespedAci + " con convocatorias distintas fecha en " + institucion + " " + FechaImserso.ToString("dd/MM/yyyy") + " y fecha en Aci " + FechaAci.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                    ojos_linea = true;



                    TIPO_ADVERTENCIA = "Diferente Convocotaria";
                    sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
                ...
                else
                {
                    ...
                    fichero_advertencias = "Este expediente " + Expediente + " existe una " + Tabla + " " + IdRererva + no_alojados_advertencias + "  de la Agencia " + AgenciaBalneario + " en estado " + estado + " del NIF " + NifHuespedAci + " con fecha distinta a la " + institucion + " " + FechaImserso.ToString("dd/MM/yyyy") + " y fecha en Aci " + FechaAci.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                    ojos_linea = true;

                    TIPO_ADVERTENCIA = "FECHA DISTINTA";
                    sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
                    ...
                    if (this.checkBox1.Checked == true)
                    {
                        ...
                        if (estado.ToUpper() == "PENDIENTE")
                        {

                            if (tipoturno.Trim() == "B (12 DIAS)")
                            {
                                this.misfuncionesBD.update("PresReservas", "res_ent_dat='" + FechaImserso.ToString(formato_datetime) + "',res_sal_dat='" + Convert.ToDateTime(FechaImserso).AddDays(11).ToString(formato_datetime) + "',res_DR_bon_str='" + res_bon_str + "',res_DR_obs_str=res_DR_obs_str +'---'+'ActulizacionFechaImserso'", " RES_GUID='" + IdRererva + "' AND  AGE_GUID='" + agencia_balneario + "'");

                            }
                            else
                            {

                                this.misfuncionesBD.update("PresReservas", "res_ent_dat='" + FechaImserso.ToString(formato_datetime) + "',res_DR_bon_str='" + res_bon_str + "',res_sal_dat='" + Convert.ToDateTime(FechaImserso).AddDays(10).ToString(formato_datetime) + "',res_DR_obs_str=res_DR_obs_str +'---'+'ActulizacionFechaImserso'", " RES_GUID='" + IdRererva + "' AND AGE_GUID='" + agencia_balneario + "'");

                            }

                            fichero_advertencias = "OJO ACTULIZACION----> Para este expediente del NIF " + NifHuespedAci + "  se actualizado su fecha la prereserva y el expediente " + IdRererva + no_alojados_advertencias + " fecha de Aci " + FechaAci.ToString("dd/MM/yyyy") + " fecha de " + institucion + " " + FechaImserso.ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                            ojos_linea = true;

                            TIPO_ADVERTENCIA = "FECHA ACTULIZACION";
                            sql = "Insert into [Hoja1$] (" + CAMPOS + ") values('" + NifHuespedAci.Trim() + "','" + formatearnif(NifSolictenteImnserso.Trim()) + "','" + nif_acomp_Aci.Trim() + "','" + formatearnif(nif_acomp_imserso.Trim()) + "','" + Expediente.Trim() + "','" + Tabla.Trim() + "','" + IdRererva.Trim() + "','" + numeroadultos.Trim() + "','" + AgenciaBalneario.Trim() + "','" + estado.Trim() + "','" + FechaAci.ToString("dd/MM/yyyy") + "','" + FechaImserso.ToString("dd/MM/yyyy") + "','" + plazassolicitadas + "'," + diasestancia.Trim() + ",'" + tipoturno + "','" + TIPO_ADVERTENCIA + "','" + fichero_advertencias + "')";
        ...
        public void InsentarLineaLog(DateTime FechaImserso, string linealog)
        {
            ...
            try
            {
                ...
                System.IO.Directory.CreateDirectory(textBox1.Text + @"\AdvertenciasImserso_" + mesfichero + "_" + anyofichero);
        ...
        //Saca todos los huespedes que no estan en 
        public void HuespedesNoEstanImserso(string mes, string year_fichero, ArrayList ListaNifsImsersoMes)
        {
            ...
            //En caso de que venga de Generelatit
            string sql_r = "";
            ...
            {
                //curFile = textBox1.Text + @"\AdvertenciasTV_" + Convert.ToString(FechaImserso.Year) + "\\Advertencias_" + fecha_actual_strg + ".xls";
                curFile = textBox1.Text + @"\AdvertenciasTV_" + mesfichero + "_" + anyofichero + "\\AvertenciasClientes_" + fecha_actual_strg + ".xls";
            }
            else
            {
                ...
                //curFile = textBox1.Text + @"\AdvertenciasImserso" + mesfichero + "_" + anyofichero + "\\Advertencias_" + fecha_actual_strg + ".xls";
                curFile = textBox1.Text + @"\AdvertenciasImserso_" + mesfichero + "_" + anyofichero + "\\AvertenciasClientes_" + fecha_actual_strg + ".xls";
            }
            ...
            if (!File.Exists(curFile))
            {
                ...
                xlWorkSheet.Cells[1, 1] = "NifSol_ACI";
                xlWorkSheet.Cells[1, 2] = "HUESPED";
                xlWorkSheet.Cells[1, 3] = "Expediente";
                xlWorkSheet.Cells[1, 4] = "Tipo";
                xlWorkSheet.Cells[1, 5] = "RESERVA";
                xlWorkSheet.Cells[1, 6] = "ADULTOS";
                xlWorkSheet.Cells[1, 7] = "AGENCIA";
                xlWorkSheet.Cells[1, 8] = "ESTADO";
                xlWorkSheet.Cells[1, 9] = "ENTRADA";
                xlWorkSheet.Cells[1, 10] = "DIAS_ESTANCIA";
                xlWorkSheet.Cells[1, 11] = "TIPO_ADVERTENCIA";
                xlWorkSheet.Cells[1, 12] = "OBSERVACION";
            ...
            if (this.checkBox3.Checked)
            {
                ...
                contratos_imsesro = "'" + System.Configuration.ConfigurationSettings.AppSettings["contrato_alta_tv"] + "','" + System.Configuration.ConfigurationSettings.AppSettings["contrato_baja_tv"] + "'";
                sql_r = "SELECT res_est_int,res_nal_bln,res_DR_bon_str,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,res_spe_adu_int,age_nom_str,DATEDIFF(day, res_ent_dat, res_sal_dat) as dias FROM Reservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=Reservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=Reservas.HUE_GUID  INNER JOIN contratos ON contratos.CTA_GUID =Reservas.CTA_GUID INNER JOIN AGENCIAs ON AGENCIAs.AGE_GUID = Reservas.AGE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND   Reservas.AGE_GUID='" + this.agencia_balneario + "' ";
            ...
            else
            {
                ...
                sql_r = "SELECT res_est_int,res_nal_bln,res_DR_bon_str,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,res_spe_adu_int,age_nom_str,DATEDIFF(day, res_ent_dat, res_sal_dat) as dias FROM Reservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=Reservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=Reservas.HUE_GUID  INNER JOIN contratos ON contratos.CTA_GUID =Reservas.CTA_GUID  INNER JOIN AGENCIAs ON AGENCIAs.AGE_GUID = Reservas.AGE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_cod_str IN (" + contratos_imsesro + ") ";
            ...
            /*Obtenemos el id del ultimo contador*/
            /// string sql_r = "SELECT res_nal_bln,res_DR_bon_str,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE epr_GUID WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID  INNER JOIN contratos ON contratos.CTA_GUID = PresReservas.CTA_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_cod_str IN (" + contratos_imsesro + ") ";
            tabla_existe_registro = this.misfuncionesBD.obtenerDatable(sql_r);
            // fichero_advertenciasclientes += "SELECT hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_GUID IN (" + contratos_imsesro + ")";


            string CAMPOS = "NifSol_ACI,HUESPED,Tipo,RESERVA,ADULTOS,AGENCIA,ESTADO,ENTRADA,DIAS_ESTANCIA,TIPO_ADVERTENCIA,OBSERVACION";
            string TIPO_ADVERTENCIA = "";
            string estado = "";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                if (nif_fichero_imserso_mes.Contains(formatearnif(Convert.ToString(dr["hue_nif_str"]))) == false)
                {
                    ...
                    //Solo Muestro esta advertencia en el caso del que balneario es administrador
            ...
            if (this.checkBox3.Checked)
            {
                ...
                contratos_imsesro = "'" + System.Configuration.ConfigurationSettings.AppSettings["contrato_alta_tv"] + "','" + System.Configuration.ConfigurationSettings.AppSettings["contrato_baja_tv"] + "'";
                sql_r = "SELECT res_spe_adu_int,age_nom_str,DATEDIFF(day, res_ent_dat, res_sal_dat) as dias,res_nal_bln,res_DR_bon_str,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE epr_GUID WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID  INNER JOIN contratos ON contratos.CTA_GUID = PresReservas.CTA_GUID  INNER JOIN AGENCIAs ON AGENCIAs.AGE_GUID = PresReservas.AGE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND  CTA_cod_str IN (" + contratos_imsesro + ") ";
            }
            else
            {


                sql_r = "SELECT res_spe_adu_int,age_nom_str,DATEDIFF(day, res_ent_dat, res_sal_dat) as dias,res_nal_bln,res_DR_bon_str,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE epr_GUID WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID  INNER JOIN contratos ON contratos.CTA_GUID = PresReservas.CTA_GUID  INNER JOIN AGENCIAs ON AGENCIAs.AGE_GUID = PresReservas.AGE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_cod_str IN (" + contratos_imsesro + ") ";
            ...
            if (this.checkBox2.Checked == true)
            {
                ...
                tabla_existe_registro = this.misfuncionesBD.obtenerDatable("SELECT   res_nal_bln,res_DR_bon_str,res_ent_dat,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 0 THEN 'Reserva' WHEN 2 THEN 'Checkin/Checkout' WHEN 3 THEN 'No Show' WHEN 4 THEN 'Anulada' ELSE '' END AS Estado FROM Reservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=Reservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=Reservas.HUE_GUID  where res_ent_dat='" + Convert.ToDateTime(this.dateTimePicker1.Value).ToString(formato_datetime) + "' AND  CTA_GUID IN (" + contratos_imsesro + ")");
            ...
            else
            {
                ...
                tabla_existe_registro = this.misfuncionesBD.obtenerDatable("SELECT  res_nal_bln,res_DR_bon_str,res_ent_dat,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 0 THEN 'Reserva' WHEN 2 THEN 'Checkin/Checkout' WHEN 3 THEN 'No Show' WHEN 4 THEN 'Anulada' ELSE '' END AS Estado FROM Reservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=Reservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=Reservas.HUE_GUID  where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_GUID IN (" + contratos_imsesro + ")");


            }


            bool error_inserto = false;
            string nif_sin_ce = "";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                //nif_sin_ce = Convert.ToString(Convert.ToInt32(Convert.ToString(dr["hue_nif_str"]).Substring(0, Convert.ToString(dr["hue_nif_str"]).Length - 2))) + Convert.ToString(dr["hue_nif_str"]).Substring(Convert.ToString(dr["hue_nif_str"]).Length-1,1);
                //MessageBox.Show(nif_sin_ce);


                expediente_estado.Clear();
                ...
                if (nif_fichero_imserso_mes.Contains(formatearnif(Convert.ToString(dr["hue_nif_str"]))) == false)
                {
                    ...
                    expediente_estado.Clear();
                    expediente_estado = ObtenerExpedienteImsersoEstado(Convert.ToString(dr["hue_nif_str"]));


                    if (Convert.ToString(dr["estado"]) != "Anulada" && Convert.ToString(expediente_estado[1]) == "" && Convert.ToString(dr["res_DR_bon_str"]) == "")
                    {

                        if (Convert.ToString(dr["res_nal_bln"]) == "True")
                        {

                            no_alojados_advertencias = " de tipo de NO ALOJADO ";

                        }
                        fichero_advertenciasclientes += Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Reserva " + Convert.ToString(dr["RES_GUID"]) + no_alojados_advertencias + "  en estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                        error_inserto = true;
                    ...
                    if (Convert.ToString(dr["estado"]) != "Anulada" && Convert.ToString(expediente_estado[1]) != "LR" && Convert.ToString(dr["res_DR_bon_str"]) == "")
                    {
                        ...
                        if (error_inserto == false)
                        {
                            //Solo Muestro esta advertencia en el caso del que balneario es administrador
                            if (this.admin_balneario == "1")
                            {

                                fichero_advertenciasclientes += Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Reserva con estado de expediente en Imserso  " + Convert.ToString(expediente_estado[1]) + " " + Convert.ToString(dr["RES_GUID"]) + no_alojados_advertencias + "  en estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                    ...
                    // MessageBox.Show(ObtenerBalnearioImserso(Convert.ToString(dr["hue_nif_str"])));


                    if (ObtenerBalnearioImserso(Convert.ToString(dr["hue_nif_str"]), Convert.ToString(expediente_estado[1])).Trim() != BalnearioImserso(Convert.ToString(this.id_balneario_seleccionado)).Trim())
                    {
                        ...
                        if (error_inserto == false)
                        {
                            ...
                            if (this.admin_balneario == "1")
                            {
                                fichero_advertenciasclientes += Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Reserva con estado de expediente  " + Convert.ToString(expediente_estado[0]) + " en Imserso  " + Convert.ToString(expediente_estado[1]) + " " + Convert.ToString(dr["RES_GUID"]) + no_alojados_advertencias + "  en estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " balneario en Imserso " + ObtenerBalnearioImserso(Convert.ToString(dr["hue_nif_str"]), Convert.ToString(expediente_estado[1])).Trim() + " balneario en aci " + BalnearioImserso(Convert.ToString(this.id_balneario_seleccionado)).Trim() + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                            }
                        }
        ...
        public void insetar_advertencias_fichero(string mes, string year_fichero)
        {
            mes_global = Convert.ToInt32(mes);



            // MessageBox.Show(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\AvertenciasClientes_" + fecha_actual_strg + ".txt");
            //System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\AvertenciasClientes_" + fecha_actual_strg + ".txt", fichero_advertenciasclientes);
            //System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\Advertencias_" + fecha_actual_strg + ".txt", fichero_advertencias);
            /*System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\log_" + fecha_actual_strg + ".txt", fichero_log);

            if (this.admin_balneario == "1")
            {

                ObtenerExpedienteImsersoEstadoMesAI();
                System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\AvertenciasClientes_ai_" + fecha_actual_strg + ".txt", fichero_advertenciascliente_ai);
            }




            System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\PreReservasPendientesReserva_" + fecha_actual_strg + ".txt", fichero_PreReservasPendientesReserva);
            System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\log_advertencias_" + fecha_actual_strg + ".txt", fichero_advertencias_log);
            //System.IO.Fil\.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\CodidosActulizarPreReservasPendientesReserva_" + fecha_actual_strg + ".txt", fichero_PreReservasPendientesReserva_Cdodigo_actulizar);
            comprobar_prereservas_repetidas(mes);
            System.IO.File.WriteAllText(textBox1.Text + @"\Advertencias" + mes + "_" + year_fichero + "\\log_repetidas_" + fecha_actual_strg + ".txt", fichero_repetidas_reservas);*/
        ...
        public ArrayList ObtenerExpedienteImsersoEstado(string nif)
        {
            ...
            string sql = "SELECT TOP 1 * FROM [expedientes] where  NIF_Solicitante_formateado='" + nif + "' AND ano='" + Convert.ToString(DateTime.Now.Year) + "' ORDER BY [Id] DESC";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                expediente_estado.Add(Convert.ToString(dr["N_Expediente"]));
                expediente_estado.Add(Convert.ToString(dr["Estado_Expediente"]));


            }


            if (expediente_estado.Count == 0)
            {

                expediente_estado.Add("");
                expediente_estado.Add("");
            }


            //MessageBox.Show(Convert.ToString(expediente_estado.Count));
        ...
        public string ObtenerBalnearioImserso(string nif, string estado)
        {
            ...
            nif_sin_ceros = nif.TrimStart(arr);
            ArrayList expediente_estado = new ArrayList();
            string balneario_asignado = "";

            DataTable tabla_existe_registro = new DataTable();
            DataSet miDataset = new DataSet();

            string sql = "SELECT * FROM [expedientes] where Estado_Expediente LIKE '" + estado + "'AND NIF_Solicitante_formateado='" + nif + "' AND ano='" + Convert.ToString(DateTime.Now.Year) + "'";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                balneario_asignado = Convert.ToString(dr["Balneario"]);
        ...
        /*Obtener balneario asignado*/


        public string BalnearioImserso(string id)
        {
            ...
            string sql = "SELECT * FROM [config_balneario_prereservas] where id='" + Convert.ToString(id) + "'";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                balneario_asignado = Convert.ToString(dr["destino"]);
        ...
        public void HuespedesNoEstanImsersoAI(string nif)
        {
            ...
            expediente_estado = ObtenerExpedienteImsersoEstado(Convert.ToString(nif));
            DataRow row;


            /*Obtenemos el id del ultimo contador*/
            tabla_existe_registro = this.misfuncionesBD.obtenerDatable("SELECT hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,PreReservaEstados.epr_des_str as Estado,prr_est_byt FROM PresReservas INNER JOIN  PreReservaEstados ON PreReservaEstados.epr_GUID=PresReservas.epr_GUID INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID where hue_nif_str='" + nif + "' AND YEAR(res_ent_dat) ='" + DateTime.Now.Year + "'");
            //ResultBlock.Text += "SELECT hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID where hue_nif_str='" + nif + "'";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                row = mytableAI.NewRow();
                // Example #4: Append new text to an existing file

                row["fecha_entrada"] = Convert.ToDateTime(dr["res_ent_dat"]);
                row["advertencia"] = Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Prereserva " + Convert.ToString(dr["RES_GUID"]) + "  esn estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + " Expediente " + institucion + " " + Convert.ToString(expediente_estado[0]) + " Estado " + Convert.ToString(expediente_estado[1]) + Environment.NewLine + Environment.NewLine; ;
                mytableAI.Rows.Add(row);
            ...
            /*Obtenemos el id del ultimo contador*/
            tabla_existe_registro = this.misfuncionesBD.obtenerDatable("SELECT   res_ent_dat,hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 0 THEN 'Reserva' WHEN 2 THEN 'Checkin/Checkout' WHEN 3 THEN 'No Show' WHEN 4 THEN 'Anulada' ELSE '' END AS Estado FROM Reservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=Reservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=Reservas.HUE_GUID  where hue_nif_str='" + nif + "'  AND YEAR(res_ent_dat) ='" + DateTime.Now.Year + "'");
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                row["fecha_entrada"] = Convert.ToDateTime(dr["res_ent_dat"]);
                row["advertencia"] = Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Reserva " + Convert.ToString(dr["RES_GUID"]) + "  esn estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + " Expediente " + institucion + "  " + Convert.ToString(expediente_estado[0]) + " Estado " + Convert.ToString(expediente_estado[1]) + Environment.NewLine + Environment.NewLine;
                mytableAI.Rows.Add(row);


                //fichero_advertenciascliente_ai += Environment.NewLine + Environment.NewLine + "OJO ----> Exite una Reserva " + Convert.ToString(dr["RES_GUID"]) + "  esn estado " + Convert.ToString(dr["estado"]) + "  --> " + Convert.ToString(dr["hue_nif_str"]) + "  " + Convert.ToString(dr["hue_des_str"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + " Expediente Imserso " + Convert.ToString(expediente_estado[0]) + " Estado " + Convert.ToString(expediente_estado[1]) + Environment.NewLine + Environment.NewLine;
                //MessageBox.Show(fichero_advertenciascliente_ai);
        ...
        public void ObtenerExpedienteImsersoEstadoMesAI()
        {
            ...
            string sql = "SELECT * FROM [expedientes] where (Turno_solicitado_uno <> '') AND (SUBSTRING(Turno_solicitado_uno, 6, 16) = '" + Convert.ToString(mes) + "') AND (SUBSTRING(Turno_solicitado_uno, 1, 4) = '" + Convert.ToString(DateTime.Now.Year) + "') AND  (Estado_Expediente LIKE 'AI')";
            ...
            foreach (DataRow dr in tabla_existe_registro.Rows)
            {
                ...
                //HuespedesNoEstanImsersoAI(formatearnif(Convert.ToString(dr["NIF_Solicitante"])));

            }

            //MessageBox.Show(Convert.ToString(mytableAI.Rows.Count));

            this.mytableAI.DefaultView.Sort = "fecha_entrada ASC";
            DataView _dv = new DataView(mytableAI);
            _dv.Sort = "fecha_entrada ASC";

            /*foreach (DataRow dr in this._dv.)
            {
                MessageBox.Show(Convert.ToString(dr["fecha_entrada"]));
                //MessageBox.Show(Convert.ToString(dr["advertencia"]));
                fichero_advertenciascliente_ai += Convert.ToString(dr["advertencia"]);
            }*/

            for (int i = 0; i < _dv.Count; i++)
            {
                //MessageBox.Show(Convert.ToString(_dv[i]["fecha_entrada"]));
                fichero_advertenciascliente_ai += Convert.ToString(_dv[i]["advertencia"]);
            }
        ...
        public void ObtenerPrereservasPendientesConReserva()
        {

            DataTable tabla_existe_registro = new DataTable();
            int total = 0;
            /*Obtenemos el id del ultimo contador*/
            tabla_existe_registro = this.misfuncionesBD.obtenerDatable("SELECT hue_des_str,hue_nif_str,res_ent_dat,RES_GUID,PreReservaEstados.epr_des_str as Estado,prr_est_byt FROM PresReservas INNER JOIN  PreReservaEstados ON PreReservaEstados.epr_GUID=PresReservas.epr_GUID INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID INNER JOIN Huespedes ON Huespedes.HUE_GUID=PresReservas.HUE_GUID where YEAR(res_ent_dat) ='" + DateTime.Now.Year + "'");
            // fichero_advertenciasclientes += "SELECT hue_nif_str,res_ent_dat,RES_GUID,CASE res_est_int WHEN 1 THEN 'Transpaso AI' WHEN 2 THEN 'SI' WHEN 3 THEN 'Pendiente' WHEN 4 THEN 'EQUIVOCADO' WHEN 5 THEN 'NO LOCALIZADO' WHEN 6 THEN 'VOLVER A LLAMAR' WHEN 7 THEN 'PROPIO' WHEN 8 THEN 'PROPIO (Otras opciones)' WHEN 9 THEN 'NO' ELSE '' END AS Estado FROM PresReservas INNER JOIN HuespedK ON HuespedK.HUE_GUID=PresReservas.HUE_GUID where YEAR(res_ent_dat) =" + year_fichero + " AND MONTH(res_ent_dat) =" + mes + " AND  CTA_GUID IN (" + contratos_imsesro + ")";

            foreach (DataRow dr in tabla_existe_registro.Rows)
            {

                if ((Convert.ToString(dr["estado"]) == "Pendiente") && (Convert.ToString(dr["prr_est_byt"]) == "1"))
                {
                    total++;

                    fichero_PreReservasPendientesReserva += Environment.NewLine + Environment.NewLine + "Linea ---> " + Convert.ToString(total) + "------> OJO ---->Exite una PReserva " + Convert.ToString(dr["RES_GUID"]) + "  esn estado " + Convert.ToString(dr["estado"]) + " Fecha de Entrada " + Convert.ToDateTime(dr["res_ent_dat"]).ToString("dd/MM/yyyy") + Environment.NewLine + Environment.NewLine;
                    this.misfuncionesBD.update("PresReservas", "PresReservas.epr_GUID='" + obtener_estado_bd("SI") + "'", " RES_GUID=" + Convert.ToString(dr["RES_GUID"]));
        ...
        public void comprobar_prereservas_repetidas(string mes)
        {


            DataTable tabla_repetidos = new DataTable();

            tabla_repetidos = this.misfuncionesBD.obtenerDatable("SELECT hue_guid,count(*) FROM [PresReservas]  WHERE MONTH(res_ent_dat)=" + mes + " GROUP BY hue_guid HAVING count(*) > 1");

            fichero_repetidas_reservas += "SELECT hue_guid,count(*) FROM [PresReservas]  WHERE MONTH(res_ent_dat)=" + mes + " GROUP BY hue_guid HAVING count(*) > 1" + Environment.NewLine + Environment.NewLine;

            foreach (DataRow dr in tabla_repetidos.Rows)
            {

                obtener_nombre_repetido_reserva(Convert.ToString(dr["hue_guid"]));
        ...
        public void obtener_nombre_repetido_reserva(string hue_guid)
        {


            DataTable tabla_repetidos = new DataTable();

            tabla_repetidos = this.misfuncionesBD.obtenerDatable("SELECT hue_des_str,HUE_COD_str,hue_guid  FROM [huespedes]  WHERE  hue_guid ='" + hue_guid + "'");


            foreach (DataRow dr in tabla_repetidos.Rows)
            {

                fichero_repetidas_reservas += "Presereserva repetida para este cliente -->" + Convert.ToString(dr["hue_guid"]) + "---- su codigo en aci es---->" + Convert.ToString(dr["HUE_COD_str"]) + Environment.NewLine + Environment.NewLine;
        ...
        public int tam_cadena(string cadena)
        {
